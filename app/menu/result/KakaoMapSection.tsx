...
import {
  ...
  ExternalLink, LocateFixed, LocateOff, Info, Phone,
} from 'lucide-react';
...
type LocationStatus = 'requesting' | 'granted' | 'low-accuracy' | 'denied' | 'fallback';
...
export default function KakaoMapSection({ keyword }: KakaoMapSectionProps) {
    ...
    (lat: number, lng: number) => {
      try {
        ...
        new window.kakao.maps.CustomOverlay({
          ...
          content:
            '<div style="width:14px;height:14px;border-radius:50%;background:#3B82F6;border:3px solid white;box-shadow:0 0 0 2px #3B82F6;"></div>',
          zIndex: 10,
        });
    ...
    const doGetPosition = () => {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          setLocationStatus(acc > 500 ? 'low-accuracy' : 'granted');
    ...
    return (
      <section className="mb-5">
        <div className="flex items-center gap-2 mb-3">
          <MapPin size={15} className="text-gray-700" strokeWidth={2} />
  ...
  return (
    <section className="mb-5">
          ...
          <span className="flex items-center gap-1 text-[10px] text-emerald-500 font-semibold">
            ...
            {locationAccuracy !== null && (
              <span className="text-[9px] text-emerald-400/80 font-normal">
                (±{locationAccuracy < 1000 ? `${locationAccuracy}m` : `${(locationAccuracy / 1000).toFixed(1)}km`})
              </span>
            )}
          </span>
        )}
      ...
      {locationStatus === 'low-accuracy' && (
        <p className="text-[11px] text-orange-400 mb-2 pl-1 leading-relaxed">
          GPS 신호가 약해 위치 정확도가 낮아요. 실외에서 새로고침하면 더 정확해져요.
        </p>
      )}
      ...
      <div
        ref={mapContainerRef}
        className="w-full h-72 rounded-2xl overflow-hidden border border-gray-200 bg-gray-50 mb-2 relative"
        ...
        <>
          <div className="flex flex-col gap-2">
            {places.map((place, index) => (
              <a
                ...
                className={`
                  flex items-start gap-3 bg-white rounded-2xl px-4 py-4
              ...
              >
                {/* 순위 배지 — 원형→사각형, amber 계열 */}
                <span className="w-7 h-7 rounded-lg bg-amber-100 text-amber-700 text-[11px] font-extrabold flex items-center justify-center flex-shrink-0 mt-0.5">
                  {index + 1}
                </span>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {place.phone && (
                    <a
                      ...
                      className="flex items-center gap-1 text-[11px] text-gray-400 mt-0.5 hover:text-gray-600 w-fit"
                    >
                ...
                {/* 거리 배지 */}
                {place.distance && (
                  <span className="text-xs font-semibold text-gray-500 flex-shrink-0 bg-gray-100 px-2 py-0.5 rounded-full mt-0.5">
          ...
          {!showAll && places.length > 3 && (
            <button
              ...
              className="sm:hidden flex items-center justify-center gap-1.5 w-full mt-2 py-3 rounded-2xl border border-gray-200 text-gray-600 text-sm font-semibold hover:bg-gray-50 active:scale-[0.98] transition-all"
